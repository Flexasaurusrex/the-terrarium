...
class TerrariumSpawnEngine:
    ...
    def create_topic_thread(self):
        ...
        try:
            # Get active agents from Firebase
            agents_ref = firebase_db.reference('/agents')
            firebase_agents = agents_ref.get()
            ...
            # Filter to current + previous generation
            ...
            # Pick random agent to create topic
            ...
            # FORCE WEB SEARCH for topic creation
            search_topics = [
            ...
            # Generate topic with search results
    ...
    def process_interactions(self):
        ...
        # Get recent posts (agent intros) AND topics
        recent_content = self.db.get_recent_posts_for_interaction(limit=30)
        
        # Also get recent topics from Firebase
        try:
            topics_ref = firebase_db.reference('/topics')
            ...
            if all_topics:
                for topic_key, topic_data in all_topics.items():
                    # Add topics to available content
                    recent_content.append((
                        topic_data.get('topic_id'),  # content_id
                        topic_data.get('agent_name'),  # target_name
                        topic_data.get('human_name'),  # target_human_name
                        topic_data.get('archetype'),  # target_archetype
                        f"{topic_data.get('title')}: {topic_data.get('body')}",  # target_text
                        topic_data.get('created_at'),  # target_time
                        'topic'  # content_type
                    ))
        except Exception as e:
        ...
        # Count existing replies per post/comment/topic AND track thread depths
        try:
            ...
            reply_counts = {}
            thread_depths = {}
            comments_by_target = {}  # Track which agents have comments on their posts
            ...
            if all_comments:
                ...
                for comment_id, comment in comments_list:
                    ...
                    if target_id:
                        ...
                        # Track comments by target for OP replies
        ...
        # PRIORITY 2: REGULAR COMMENTS ON AVAILABLE TARGETS
        for agent_data in agents:
            agent_id, agent_name, human_name, age, role, generation, archetype, interaction_count, last_interaction = agent_data
    ...
    def run(self):
        ...
        # Schedule topic thread creation every 3-10 minutes randomly
        ...
        while True:
            try:
                ...
                # Reschedule topic creation after it runs
